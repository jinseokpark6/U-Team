...
{
	[self notifyDelegateOfDisableTap:sender];
}
...
- (void)notifyDelegateOfDisableTap: (id)sender
{
    if ([self.delegate respondsToSelector:@selector(addressBarViewControllerDidSelectWhileDisabled:)]) {
		[self.delegate addressBarViewControllerDidSelectWhileDisabled:self didTapBarWhileDisabled:sender];
    }
}
